...
export class StorageService extends BaseService {
  ...
  async setToStorage(key: string, val: any) {
    let value: string = '';
  ...
  async getTokenFromStorage(): Promise<string> {
    ...
    if (remMe && remMe == true)
      return await this.getFromStorage(AppConstants.DbKeys.ACCESS_TOKEN);
    return await this.getFromSessionStorage(AppConstants.DbKeys.ACCESS_TOKEN);
  }
