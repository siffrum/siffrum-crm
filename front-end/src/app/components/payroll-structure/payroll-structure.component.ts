  ...
  implements OnInit
{
  ...
  async addGenericPayrollComponent(payrollForm: NgForm) {
    ...
    try {
      ...
      // ✅ FIX: total % should NOT exceed 100
      const currentSum = (this.viewModel.genericPayrolls || []).reduce(
        (sum, obj: any) => sum + (Number(obj?.percentage) || 0),
        0
      );

      const newValue = Number(this.viewModel.selectedGenericPayroll?.percentage) || 0;
  ...
  async openModal(id: number) {
    this.viewModel.selectedGenericPayroll = new ClientGenericPayrollComponentSM();

    if (id > 0) {
      await this.getGenericPayrollComponentsById(id);
